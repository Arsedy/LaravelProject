  ...
  <div class="col-lg-8 mx-auto">
    ...
    <div class="card border-0 shadow-sm bg-dark text-white">
      <div class="card-header border-bottom border-secondary py-3 bg-dark">
        <h4 class="mb-0 text-white fw-bold">Edit Category: <span class="text-info">{{ $category->title }}</span></h4>
      </div>
      <div class="card-body p-4">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" id="categoryForm">
          ...
          <div class="mb-3">
            ...
            <select name="parent_id" id="parent_id" class="form-select bg-secondary text-white border-secondary @error('parent_id') is-invalid @enderror">
              ...
              @foreach($parentCategories as $parent)
                <option value="{{ $parent->id }}" class="bg-dark" {{ old('parent_id', $category->parent_id) == $parent->id ? 'selected' : '' }}>
                  {{ $parent->title }}
                </option>
              @endforeach
            </select>
          ...
          <!-- Image Upload with current preview and JS Preview -->
          <div class="mb-3">
            ...
            <div class="mt-3 {{ $category->image ? '' : 'd-none' }}" id="previewContainer">
              <label class="form-label text-white-50 fs-8" id="previewLabel">
                {{ $category->image ? 'Current Image:' : 'Image Preview:' }}
              </label>
              <div>
                <img 
                  id="imagePreview" 
          ...
          <div class="mb-4">
            ...
            <select name="status" id="status" class="form-select bg-secondary text-white border-secondary @error('status') is-invalid @enderror">
              <option value="1" class="bg-dark" {{ old('status', $category->status ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
              <option value="0" class="bg-dark" {{ old('status', $category->status ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
            </select>
            @error('status')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
